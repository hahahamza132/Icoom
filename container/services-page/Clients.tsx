...
export default function Clients() {
	...
	return (
		<section className="w-full padding-y bg-gradient-to-b from-white to-blue-50">
			<div className="w-full padding-x mb-[50px]">
				<motion.h1 
				...
				<motion.p
					...
					viewport={{ once: true }}
				>
					Hear from businesses we've helped transform their communications
				</motion.p>
			</div>
					...
					<div className="w-full flex items-center justify-between py-[10px] padding-x">
						<div className="w-[50%] flex items-center">
							<div className="w-[40%] sm:w-auto xm:w-auto">
								<Link
									href={item.href}
									className="small-text font-normal font-NeueMontreal text-blue-800 hover:text-blue-600 transition-colors duration-300">
									{item.website}
								</Link>
							</div>
							<div className="w-auto sm:hidden xm:hidden">
								<motion.h3
									className={`small-text font-normal font-NeueMontreal text-blue-700 ${
						...
						<div className="w-[50%] flex justify-between items-center">
							<div className="w-[40%] sm:w-auto xm:w-auto">
								<h3 className="small-text font-normal font-NeueMontreal text-blue-800">
									{item.name}
								</h3>
							</div>
							<div className="w-[10%] sm:w-auto xm:w-auto flex items-end justify-end">
								<motion.button
									...
									whileTap={{ scale: 0.95 }}
								>
									{activeAccordion === item.id ? "hide" : "read"}
								</motion.button>
							</div>
						</div>
					</div>
							...
							<AnimatePresence>
								{activeAccordion === item.id && (
									<motion.div
										...
										<div className="flex flex-col gap-[20px] py-[30px] bg-blue-50/50 rounded-xl p-6 border border-blue-100">
											<div className="w-[130px] h-[130px]">
												<Image
													src={item.src}
													alt="client"
													className="w-full h-full object-cover rounded-[10px] border border-blue-200"
												/>
											</div>
											<div className="">
												<p className="small-text tracking-wider font-normal font-NeueMontreal text-blue-800">
													{item.review}
												</p>
											</div>
										</div>
